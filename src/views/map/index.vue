    ...
    <div class="sidebar">
      <Tabs class="tabs" v-model="tab">
        <TabPane label="底图切换" name="base">
          <div class="content">
            <make-selectable
              v-for="{ name, label, img } in sortedBaseLayers"
              :key="name"
        ...
        <TabPane label="图层控制" name="overlay">
          <layer-list :items="sortedOverLayers" v-model="selectedOverLayerNames"></layer-list>
        </TabPane>
      </Tabs>
    </div>
...
export default {
  ...
  computed: {
    sortedBaseLayers () {
      return _.sortBy(this.baseLayers, 'zIndex')
    },
    sortedOverLayers () {
      return _.sortBy(this.overLayers, 'zIndex')
    }
  },
  ...
  methods: {
    ...
    registerLayerToggleListener () {
      // 批量切换图层实例显示状态
      const toggleLayerVisible = async (layers, selected) => {
        for (const layer of layers) {
          if (selected.includes(layer.name)) {
            // 第一次展示时调用图层创建方法，后续直接调用图层实例显隐
            if (!layer.layerInstance) {
              layer.layerInstance = await layer.layerCreater()
      ...
      // 递归获取所有层级所有图层
      const deepLayers = (layers, all = []) => {
        for (const layer of layers) {
          all.push(layer) && layer.children && deepLayers(layer.children, all)
        }
      ...
      this.$watch('selectedBaseLayerNames', v => toggleLayerVisible(deepLayers(this.baseLayers), v), { immediate: true, deep: true })
      this.$watch('selectedOverLayerNames', v => toggleLayerVisible(deepLayers(this.overLayers), v), { immediate: true, deep: true })
    },
    /**
     * 添加地图插件
